...
temp_fused_dir = os.path.join(workspace_dir, "scratch/temp_fused_storybook_lcm")
temp_coreml_dir = os.path.join(workspace_dir, "scratch/temp_coreml_storybook")
os.makedirs("scratch", exist_ok=True)

print("\n=== Step 1: Merging SD 1.5 with StorybookRedmond LoRA and LCM-LoRA ===")
try:
...
# Using standard SD 1.5 base model (4-channel, supporting T2I and I2I/Inpainting)
base_model_id = "runwayml/stable-diffusion-v1-5"
lcm_lora_id = "latent-consistency/lcm-lora-sdv1-5"
storybook_lora_id = "artificialguybr/storybookredmond-1-5-version-storybook-kids-lora-style-for-sd-1-5"
storybook_file = "StoryBookRedmond15-KidsRedmAF-KidsBook.safetensors"
...
print(f"📥 Loading StorybookRedmond LoRA weights: {storybook_lora_id}")
pipe.load_lora_weights(storybook_lora_id, weight_name=storybook_file, adapter_name="storybook")

print(f"📥 Loading LCM-LoRA weights: {lcm_lora_id}")
pipe.load_lora_weights(lcm_lora_id, adapter_name="lcm")

print(f"🧪 Fusing LoRA adapters on {device}...")
# We set weights: storybook = 0.85 (strong illustration style), lcm = 1.0 (4-step generation)
pipe.set_adapters(["storybook", "lcm"], adapter_weights=[0.85, 1.0])
pipe.fuse_lora()
...
print("\n=== Step 2: Converting fused model to CoreML (with 8-bit Quantization) ===")
...
# Construct conversion command (converting VAE Encoder as well to support I2I!)
...
output_zip = os.path.join(workspace_dir, "storybook_lcm.zip")
...
if is_mac:
    ...
    # On Linux (GitHub Actions ubuntu-latest), we have .mlpackage folders.
